<?php
	...
	$donations = $auth_user->maturedDonations();
	...
	if(isset($_POST["cancel"])) {
		...
		for($i = 0; $i < $cnt; $i++) {
			$rowID = $_POST['tick'][$i];
			try	{
				$stmt = $auth_user->runQuery("UPDATE donations SET donor_status='Canceled' WHERE donor_id=:rowID limit 1");
		...
		for($i = 0; $i < $cnt; $i++) {
			$rowID = $_POST['tick'][$i];
			try	{
				$stmt = $auth_user->runQuery("DELETE FROM donations WHERE donor_id=:rowID limit 1");
				$stmt->execute(array(':rowID'=>$rowID));				
			}
			catch(PDOException $e) {
				echo $e->getMessage();
