<?php
	...
	if(isset($_GET["id"]) AND $_GET["id"] != ""){
		...
		$auth_user->redirect(BASE_URL.'administrator/supports');
		exit();
	}
	$conversations = $auth_user->msgDetails($senderID);
	...
	if(isset($_POST['message'])) {				 
		...
		if(!isset($errorMSG)){
			...
			try	{
				...
				$stmt->execute(array(':senderID'=>$senderID, ':message'=>$message));
				...
				$auth_user->redirect(BASE_URL.'administrator/support-details?id='.$id.'&sender='.$senderID.'&thanks');
				exit();
			}
			catch(PDOException $e) {
				echo $e->getMessage();
