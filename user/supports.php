<?php
	...
	$supports = $auth_user->supports($loginID);
    ...
    if(isset($_GET["deleteID"])){
        ...
        try {
            ...
            $auth_user->redirect(BASE_URL.'user/supports?Deleted');
        }
                            ...
                            <div class="table-responsive">
                                ...
                                id="dataTables-example">
                                    ...
                                    <tbody style="font-size: 12px; font-weight: 800;">
                                    	<?php 
                                            if (!empty($supports)) {
                                                foreach($supports as $enquiry) {?>
										<tr>
                                            <td><?php echo $enquiry["sender_name"];?></td>
                                            <td>
                                            <?php if($enquiry["status"] == "Unread"){?><a style="font-weight: 800" href="<?php echo BASE_URL.'user/support-details?id='.$enquiry["id"].'&sender='.$enquiry["sender_id"]; ?>"><?php echo $enquiry["subject"];?>... </a>
                                            <?php }else{?>
                                            	<a href="<?php echo BASE_URL.'user/support-details?id='.$enquiry["id"].'&sender='.$enquiry["sender_id"];?>"><?php echo $enquiry["subject"];?>... </a>
                                            <?php }?>
                                            </td>
                                            <td><?php echo timeAgo($enquiry["date_sent"]);?></td>
                                            <td><a href="<?php echo BASE_URL.'user/supports?deleteID='.$enquiry["id"] ?>">Delete</a></td>
                                        </tr>
                                        <?php  } }?>
                                    </tbody>
                                </table>
